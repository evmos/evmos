...
func AddTxFlags(cmd *cobra.Command) (*cobra.Command, error) {
	...
	cmd.PersistentFlags().String(flags.FlagFees, "", "Fees to pay along with transaction; eg: 10aevmos. Set to 'auto' to calculate sufficient fees and gas automatically. By omitting this flag, the tx uses the required fees (but not the required gas)")
