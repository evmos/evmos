...
var (
	...
	regexEpoch           = `[1-9][0-9]*`
	eidon-chainChainID         = regexp.MustCompile(fmt.Sprintf(`^(%s)%s(%s)%s(%s)$`,
		regexChainID,
...
func IsValidChainID(chainID string) bool {
	...
	return eidon-chainChainID.MatchString(chainID)
}
...
func ParseChainID(chainID string) (*big.Int, error) {
	...
	matches := eidon-chainChainID.FindStringSubmatch(chainID)
