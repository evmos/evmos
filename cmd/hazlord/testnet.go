...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices, fmt.Sprintf("0.000006%s", ethermint.AttoPlanet), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.01photino,0.001stake)")
...
Example:
	hazlord testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "hazlord", "Home directory of the node's daemon configuration")
...
Example:
	hazlord testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(ethermint.AttoPlanet, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(ethermint.AttoPlanet, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(ethermint.AttoPlanet)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, ethermint.AttoPlanet, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
