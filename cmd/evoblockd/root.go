...
func NewRootCmd() (*cobra.Command, params.EncodingConfig) {
	...
	initClientCtx := client.Context{}.
		...
		WithKeyringOptions(evoblockkr.Option()).
		WithViper(EnvPrefix)
...
func (a appCreator) appExport(
	...
	if height != -1 {
		evoblockApp = app.NewEvoblock(logger, db, traceStore, false, map[int64]bool{}, "", uint(1), a.encCfg, appOpts)

		if err := evoblockApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, err
		}
	} else {
		evoblockApp = app.NewEvoblock(logger, db, traceStore, true, map[int64]bool{}, "", uint(1), a.encCfg, appOpts)
	}

	return evoblockApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs)
}
