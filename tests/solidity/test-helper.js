...
function checkTestEnv() {
  ...
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network evmos --allowTests=test1,test2', 'run only test1 and test2 using evmos network')
    .help('h').alias('h', 'help')
    ...
    .boolean('verbose-log').describe('verbose-log', 'print evmosd output, default false')
    .argv;
...
function setupNetwork({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    const evmosdProc = spawn('./init-test-node.sh', {
      cwd: __dirname,
