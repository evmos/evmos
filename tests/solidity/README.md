...
**Prerequisite**: in the repo's root, run `make install` to install the `evmosd` and `evmosd` binaries. When done, come back to this directory.
...
To run the tests, you can use the `test-helper.js` utility to test all suites under `ganache` or `evmos` network. The `test-helper.js` will help you spawn an `evmosd` process before running the tests.

You can simply run `yarn test --network evmos` to run all tests with evmos network, or you can run `yarn test --network ganache` to use ganache shipped with truffle. In most cases, there two networks should produce identical test results.

If you only want to run a few test cases, append the name of tests following by the command line. For example, use `yarn test --network evmos basic` to run the `basic` test under `evmos` network.

If you need to take more control, you can also run `evmosd` using:
...
Keep the terminal window open, go into any of the tests and run `yarn test-evmos`. You should see `evmosd` accepting transactions and producing blocks. You should be able to query for any transaction via:

- `evmosd query tx <cosmos-sdk tx>`
...
The [`init-test-node.sh`](./init-test-node.sh) script sets up evmos with the following accounts:
...
Running `evmosd keys list --keyring-backend=test` should output:
