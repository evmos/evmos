...
def test_gas_deployment(geth, evmos):
    ...
    evmos_tx = build_deploy_contract_tx(evmos.w3, info)

    # estimate tx gas
    geth_gas_estimation = geth.w3.eth.estimate_gas(geth_tx)
    evmos_gas_estimation = evmos.w3.eth.estimate_gas(evmos_tx)

    assert geth_gas_estimation == evmos_gas_estimation

    # sign and send tx
    geth_contract_receipt = send_transaction(geth.w3, geth_tx)
    evmos_contract_receipt = send_transaction(evmos.w3, evmos_tx)
...
def test_gas_call(geth, evmos):
    ...
    evmos_gas_estimation = evmos.w3.eth.estimate_gas(evmos_tx)
    evmos_call_receipt = send_transaction(evmos.w3, evmos_tx)
