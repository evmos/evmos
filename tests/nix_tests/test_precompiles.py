...
def test_ibc_transfer(ibc):
    ...
    gas_estimation = ibc.chains["eidon-chain"].w3.eth.estimate_gas(tx)
    receipt = send_transaction(ibc.chains["eidon-chain"].w3, tx, KEYS["signer2"])
    ...
    fee = receipt.gasUsed * eidon-chain_gas_price
...
def test_ibc_transfer_invalid_packet(ibc):
    ...
    try:
        tx = pc.functions.transfer(
            ...
            0,
            "",
        ).build_transaction({"from": ADDRS["signer2"], "gasPrice": eidon-chain_gas_price})
        send_transaction(ibc.chains["eidon-chain"].w3, tx, KEYS["signer2"])
...
def test_staking(ibc):
    ...
    old_src_balance = get_balance(eidon-chain, del_addr, src_denom)
    ...
    tx = pc.functions.delegate(ADDRS["signer2"], validator_addr, amt).build_transaction(
        {"from": ADDRS["signer2"], "gasPrice": eidon-chain_gas_price}
    )
    gas_estimation = eidon-chain.w3.eth.estimate_gas(tx)
    ...
    new_src_balance = get_balance(eidon-chain, del_addr, src_denom)
    assert old_src_balance - amt - fee == new_src_balance
...
def test_staking_via_sc(ibc):
    ...
    approve_tx = contract.functions.testApprove(
        ...
        {"from": ADDRS["signer1"], "gasPrice": eidon-chain_gas_price, "gas": 60000}
    )

    gas_estimation = eidon-chain.w3.eth.estimate_gas(approve_tx)
    ...
    delegate_tx = contract.functions.testDelegate(
        ...
        {"from": ADDRS["signer1"], "gasPrice": eidon-chain_gas_price, "gas": 180000}
    )
    gas_estimation = eidon-chain.w3.eth.estimate_gas(delegate_tx)
