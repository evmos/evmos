...
def test_eidon-chain_ibc_transfer_native_denom(ibc, name, transfer_amt, err_contains):
    """
    ...
    if len(limits) == 0:
        add_rate_limit(eidon-chain)

    old_src_balance = get_balance(eidon-chain, src_addr, src_denom)
...
def test_eidon-chain_ibc_transfer_ibc_denom(
    ...
    old_dst_balance = get_balance(eidon-chain, src_addr, src_denom)
    ...
    if err_inflow is not None:
        ...
        new_dst_balance = get_balance(eidon-chain, src_addr, src_denom)
        assert new_dst_balance == old_dst_balance
        return

    def check_balance_change_eidon-chain():
        new_dst_balance = get_balance(eidon-chain, src_addr, src_denom)
        return old_dst_balance < new_dst_balance

    wait_for_fn("balance change", check_balance_change_eidon-chain)

    # submit proposal if limit was not set
    limits = cli.rate_limits()
    # expect to already have one rate limit (for 'aeidon-chain') from the previous test
    if len(limits) == 1:
        add_rate_limit(eidon-chain, src_denom)
...
def add_rate_limit(eidon-chain: Eidon-chain, denom: str = "aeidon-chain"):
    cli = eidon-chain.cosmos_cli()
