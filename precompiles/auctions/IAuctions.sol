...
address constant AUCTIONS_PRECOMPILE_ADDRESS = 0x0000000000000000000000000000000000000900;
...
interface IAuctions {
    ...
    // @param round - the round of the auction.
    // @param amount - the amount in Evmos tokens for the bid.
    event Bid(address indexed sender, uint64 indexed round, uint256 amount);
    ...
    event CoinDeposit(address indexed sender, uint64 indexed round, address denom, uint256 amount);
    ...
    event AuctionEnd(address indexed winner, uint64 indexed round, Coin[] coins, uint256 burned);
