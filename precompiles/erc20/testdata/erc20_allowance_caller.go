// Copyright Tharsis Labs Ltd.(Evmos)
// SPDX-License-Identifier:ENCL-1.0(https://github.com/evmos/evmos/blob/main/LICENSE)

package testdata

import (
	_ "embed" // embed compiled smart contract
	"encoding/json"

	evmtypes "github.com/evmos/evmos/v16/x/evm/types"
)

var (
	//go:embed ERC20AllowanceCaller.json
	ERC20AllowanceCaller []byte

	// ERC20AllowanceCallerHardhatContract is the compiled contract calling the staking precompile
	// generated by HardHat.
	ERC20AllowanceCallerHardhatContract evmtypes.HardhatCompiledContract

	// ERC20AllowanceCallerContract is the compiled contract calling the staking precompile
	ERC20AllowanceCallerContract evmtypes.CompiledContract
)

func init() {
	err := json.Unmarshal(ERC20AllowanceCaller, &ERC20AllowanceCallerHardhatContract)
	if err != nil {
		panic(err)
	}

	ERC20AllowanceCallerContract, err = ERC20AllowanceCallerHardhatContract.ToCompiledContract()
	if err != nil {
		panic(err)
	}

	if len(ERC20AllowanceCallerContract.Bin) == 0 {
		panic("failed to load smart contract that calls erc20 precompile allowance functionality")
	}
}
