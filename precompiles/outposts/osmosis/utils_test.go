...
func (s *PrecompileTestSuite) SetupWithGenesisValSet(
...
func (s *PrecompileTestSuite) NewTestChainWithValSet(
...
func (s *PrecompileTestSuite) registerERC20Coins() {
	...
	osmoMetadata := banktypes.Metadata{
		...
		DenomUnits: []*banktypes.DenomUnit{
			{
				// This denom with exponent 0 is required by the field Base
	...
	err = s.app.BankKeeper.MintCoins(s.ctx, inflationtypes.ModuleName, sdk.NewCoins(osmo))
	s.Require().NoError(err)
	err = s.app.BankKeeper.SendCoinsFromModuleToAccount(s.ctx, inflationtypes.ModuleName, s.address.Bytes(), sdk.NewCoins(osmo))
	s.Require().NoError(err)
