...
func (s *PrecompileTestSuite) TestSwap() {
	...
	gas := uint64(2_000)
	senderAddress := s.keyring.GetAddr(0)
	...
	wevmosAddress := common.HexToAddress(testutils.WEVMOSContractTestnet)
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() []interface{} {
				...
				err = evmosutil.FundAccountWithBaseDenom(s.unitNetwork.GetContext(), s.unitNetwork.App.BankKeeper, sender, 1000000000000000000)
