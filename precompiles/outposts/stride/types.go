...
func CreateLiquidStakeEvmosPacket(args []interface{}) (common.Address, common.Address, *big.Int, string, error) {
	if len(args) != 4 {
	...
	erc20Addr, ok := args[1].(common.Address)
	if !ok {
		return common.Address{}, common.Address{}, nil, "", fmt.Errorf(cmn.ErrInvalidType, "erc20Addr", "", args[1])
	}
	...
	if !ok {
		return common.Address{}, common.Address{}, nil, "", fmt.Errorf(cmn.ErrInvalidType, "receiverAddress", "", args[3])
	}

	// Check if the receiver address has stride before
	// TODO: This might be unnecessary
	if receiverAddress[:6] != "stride" {
		return common.Address{}, common.Address{}, nil, "", fmt.Errorf("receiverAddress is not a stride address")
	}

	// Check if account is a valid bech32 address
	_, err := AccAddressFromBech32(receiverAddress, "stride")
	if err != nil {
		return common.Address{}, common.Address{}, nil, "", sdkerrors.ErrInvalidAddress.Wrapf("invalid bech32 address: %s", err)
	}

	return sender, erc20Addr, amount, receiverAddress, nil
}
