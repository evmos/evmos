// Copyright Tharsis Labs Ltd.(Evmos)
// SPDX-License-Identifier:ENCL-1.0(https://github.com/evmos/evmos/blob/main/LICENSE)

package testdata

import (
	_ "embed" // embed compiled smart contract
	"encoding/json"

	evmtypes "github.com/evmos/evmos/v16/x/evm/types"
)

var (
	//go:embed StakingCaller.json
	StakingCallerJSON []byte

	// StakingCallerHardhatContract is the compiled contract calling the staking precompile
	// generated by HardHat.
	StakingCallerHardhatContract evmtypes.HardhatCompiledContract

	// StakingCallerContract is the compiled contract calling the staking precompile
	StakingCallerContract evmtypes.CompiledContract
)

func init() {
	err := json.Unmarshal(StakingCallerJSON, &StakingCallerHardhatContract)
	if err != nil {
		panic(err)
	}

	StakingCallerContract, err = StakingCallerHardhatContract.ToCompiledContract()
	if err != nil {
		panic(err)
	}

	if len(StakingCallerContract.Bin) == 0 {
		panic("failed to load smart contract that calls staking precompile")
	}
}
