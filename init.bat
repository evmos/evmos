...
#TRACE="--trace"
set TRACE=""
set HOME=%USERPROFILE%\.berachaind
echo %HOME%
...
rem Change parameter token denominations to abera
cat %GENESIS% | jq ".app_state[\"staking\"][\"params\"][\"bond_denom\"]=\"abera\""   >   %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"crisis\"][\"constant_fee\"][\"denom\"]=\"abera\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"gov\"][\"deposit_params\"][\"min_deposit\"][0][\"denom\"]=\"abera\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"mint\"][\"params\"][\"mint_denom\"]=\"abera\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
...
berachaind start --pruning=nothing %TRACE% --log_level %LOGLEVEL% --minimum-gas-prices=0.0001abera