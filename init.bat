...
rem evoblock compile on windows
...
#TRACE="--trace"
set TRACE=""
set HOME=%USERPROFILE%\.evoblockd
echo %HOME%
...
rem Set moniker and chain-id for Evoblock (Moniker can be anything, chain-id must be an integer)
evoblockd init %MONIKER% --chain-id %CHAINID% 

rem Change parameter token denominations to aEVO
cat %GENESIS% | jq ".app_state[\"staking\"][\"params\"][\"bond_denom\"]=\"aEVO\""   >   %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"crisis\"][\"constant_fee\"][\"denom\"]=\"aEVO\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"gov\"][\"deposit_params\"][\"min_deposit\"][0][\"denom\"]=\"aEVO\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"mint\"][\"params\"][\"mint_denom\"]=\"aEVO\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
