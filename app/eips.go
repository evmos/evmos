...
func enable0000(jt *vm.JumpTable) {
	// Gas cost changes
	jt[vm.CREATE].SetConstantGas(params.CreateGas * multiplier)
	jt[vm.CREATE2].SetConstantGas(params.CreateGas * multiplier)
}
...
func enable0001(jt *vm.JumpTable) {
	// Gas cost changes
	jt[vm.CALL].SetConstantGas(params.CallGasEIP150 * multiplier)
}
...
func enable0002(jt *vm.JumpTable) {
	// Gas cost changes
	jt[vm.SSTORE].SetConstantGas(params.SstoreSetGas * multiplier)
}
