...
func (suite *AnteTestSuite) TestDeductFeeDecorator() {
	var (
		...
		// General setup - new accounts without any funds/delegations
	...
	testcases := []deductFeeDecoratorTestCase{
		...
		},
		{
			...
			postCheck: func() {
				...
				balance := suite.network.App.BankKeeper.GetBalance(suite.network.GetContext(), addr, utils.BaseDenom)
				suite.Require().Equal(zero, balance.Amount, "expected balance to be zero")

				// there should be no rewards
				rewards, err := testutil.GetTotalDelegationRewards(suite.network.GetContext(), suite.network.App.DistrKeeper, addr)
		...
		},
		{
			...
			malleate: func() {
				...
				err := testutil.FundAccountWithBaseDenom(suite.network.GetContext(), suite.network.App.BankKeeper, fgAddr, initBalance.Int64())
				suite.Require().NoError(err)
				// grant the fees
				grant := sdk.NewCoins(sdk.NewCoin(
					utils.BaseDenom, initBalance,
				))
				err = suite.network.App.FeeGrantKeeper.GrantAllowance(suite.network.GetContext(), fgAddr, addr, &feegrant.BasicAllowance{
					SpendLimit: grant,
				})
				suite.Require().NoError(err)
			},
		...
		},
		{
			...
			malleate: func() {
				...
				dfd = cosmosante.NewDeductFeeDecorator(
					suite.network.App.AccountKeeper, suite.network.App.BankKeeper, suite.network.App.DistrKeeper, nil, suite.network.App.StakingKeeper, nil,
				)
			},
		},
	}
	...
	for _, tc := range testcases {
		suite.Run(tc.name, func() {
			var args txfactory.CosmosTxArgs
			suite.SetupTest()

			// make the setup for the test case
			dfd, args = suite.setupDeductFeeDecoratorTestCase(addr, tc)

			if tc.malleate != nil {
				tc.malleate()
			}
			if tc.checkTx {
				suite.network.WithCheckTxContext()
			}

			// Create a transaction out of the message
			tx, err := suite.factory.BuildCosmosTx(priv, args)
			suite.Require().NoError(err, "failed to create transaction")

			// run the ante handler
			_, err = dfd.AnteHandle(suite.network.GetContext(), tx, tc.simulate, testutil.NextFn)
