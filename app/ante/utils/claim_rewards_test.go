...
func (suite *AnteTestSuite) TestClaimStakingRewardsIfNecessary() {
	...
	testcases := []struct {
	...
	}{
		{
			...
			malleate: func(addr sdk.AccAddress) {
				...
				ctx, err = testutil.PrepareAccountsForDelegationRewards(
					suite.T(), ctx, app, addr, math.ZeroInt(), math.NewInt(1e18),
				)
		...
		},
		{
			...
			malleate: func(addr sdk.AccAddress) {
				...
				var err error
				ctx, err = testutil.PrepareAccountsForDelegationRewards(
					suite.T(), ctx, app, addr, math.ZeroInt(), math.NewInt(1e14), math.NewInt(2e14),
				)
				suite.Require().NoError(err, "failed to prepare accounts for delegation rewards")
			},
		...
		},
		{
			...
			malleate: func(addr sdk.AccAddress) {
				var err error
				ctx, err = testutil.PrepareAccountsForDelegationRewards(
					suite.T(), ctx, app, addr, math.NewInt(1e15), math.NewInt(1e18),
				)
				suite.Require().NoError(err, "failed to prepare accounts for delegation rewards")
			},
			amount: sdk.Coins{sdk.Coin{Denom: utils.BaseDenom, Amount: math.NewInt(1000)}},
	...
	for _, tc := range testcases {
		suite.Run(tc.name, func() {
			...
			err := anteutils.ClaimStakingRewardsIfNecessary(ctx, app.BankKeeper, app.DistrKeeper, app.StakingKeeper, addr, tc.amount)
