...
func NewDynamicFeeChecker(k DynamicFeeEVMKeeper) anteutils.TxFeeChecker {
	return func(ctx sdk.Context, feeTx sdk.FeeTx) (sdk.Coins, int64, error) {
		// TODO: the fee in the genesis transaction meet the baseFee and minGasPrice in the feemarket, we can remove this code
		if ctx.BlockHeight() == 0 {
			// genesis transactions: fallback to min-gas-price logic
			// currently, the baseFee in the genesis.json generated by `evmosd init` default is 1gwei,
			// but the gasPrice of the genesis transaction signed by `evmosd gentx` default to 0.
			// So in theory we should modify the `evmosd gentx` command and pass in a reasonable gasPrice.
