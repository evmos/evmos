...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/hazlorlabs/hsc/types"
	godoc -http=:6060
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/hazlorlabs/hsc
.PHONY: format
...
PACKAGE_NAME:=github.com/hazlorlabs/hsc
