...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
EVO_BINARY = evoblockd
EVO_DIR = evoblock
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/evoblockchain/evoblock.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=evoblock \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(EVO_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm evoblock || true
	# create a new container from the latest image
	$(DOCKER) create --name evoblock -t -i ${DOCKER_IMAGE}:latest evoblock
	# move the binaries to the ./build directory
	mkdir -p ./build/
	$(DOCKER) cp evoblock:/usr/bin/evoblockd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/evoblockchain/evoblock/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(EVO_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(EVO_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(EVO_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(EVO_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/evoblockchain/evoblock
.PHONY: format
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(EVO_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/evoblock:Z evoblock/node "./evoblockd testnet init-files --v 4 -o /evoblock --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\evoblockd:/evoblock\Z evoblock/node "./evoblockd tendermint unsafe-reset-all --home=/evoblock"
	@docker run --rm -v $(CURDIR)\build\node1\evoblockd:/evoblock\Z evoblock/node "./evoblockd tendermint unsafe-reset-all --home=/evoblock"
	@docker run --rm -v $(CURDIR)\build\node2\evoblockd:/evoblock\Z evoblock/node "./evoblockd tendermint unsafe-reset-all --home=/evoblock"
	@docker run --rm -v $(CURDIR)\build\node3\evoblockd:/evoblock\Z evoblock/node "./evoblockd tendermint unsafe-reset-all --home=/evoblock"
else
	@docker run --rm -v $(CURDIR)/build/node0/evoblockd:/evoblock:Z evoblock/node "./evoblockd tendermint unsafe-reset-all --home=/evoblock"
	@docker run --rm -v $(CURDIR)/build/node1/evoblockd:/evoblock:Z evoblock/node "./evoblockd tendermint unsafe-reset-all --home=/evoblock"
	@docker run --rm -v $(CURDIR)/build/node2/evoblockd:/evoblock:Z evoblock/node "./evoblockd tendermint unsafe-reset-all --home=/evoblock"
	@docker run --rm -v $(CURDIR)/build/node3/evoblockd:/evoblock:Z evoblock/node "./evoblockd tendermint unsafe-reset-all --home=/evoblock"
endif
...
PACKAGE_NAME:=github.com/evoblockchain/evoblock
