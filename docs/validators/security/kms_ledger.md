...
- Edit `addr` to point to your `cantod` instance.
- Adjust `chain-id` to match your `.cantod/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `cantod`.
...
The KMS may complain that it cannot connect to `cantod`. That is fine, we will fix it in the next section.
...
You need to enable KMS access by editing `.cantod/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `cantod`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell cantod the key that we've got in the previous section.

```bash
cantod gentx --name kmsval --pubkey <pub_key>
```

Now start `cantod`. You should see that the KMS connects and receives a signature request.
