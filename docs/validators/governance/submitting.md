...
For information on how to use `cantod` binary to submit an on-chain proposal through the governance module, please refer to the [quickstart](../quickstart/binary.md) documentation.

### CLI

This is the command format for using `cantod` (the command-line interface) to submit your proposal on-chain:

```bash
cantod tx gov submit-proposal \
  --title=<title> \
...
1. `cantod` is the command-line interface client that is used to send transactions and query Evmos
...
After posting your transaction, your command line interface (`cantod`) will provide you with the transaction's hash, which you can either query using cantod or by searching the transaction hash using [Mintscan](https://www.mintscan.io/evmos) or any block explorer.
...
```bash
cantod q gov proposals --depositor evmos1hxv7mpztvln45eghez6evw2ypcw4vjmsmr8cdx
```

Once you have the proposal ID, this is the command to deposit extra tokens:

```bash
cantod tx gov deposit <proposal-id> <deposit> --from <name>
```
