...
```bash
cantod keys add --multisig=name1,name2,name3[...] --multisig-threshold=K new_key_name
```
...
```bash
cantod keys add --multisig=p1,p2,p3 --multisig-threshold=2 multisig_address
cantod keys add --multisig=p2,p3,p1 --multisig-threshold=2 multisig_address
```

Multisig addresses can also be generated on-the-fly and printed through the which command:

```bash
cantod keys show --multisig-threshold=K name1 name2 name3 [...]
```
...
```sh
cantod tx broadcast signedTx.json \
    --chain-id=evmos_9000-4 \
    --broadcast-mode=block
```
