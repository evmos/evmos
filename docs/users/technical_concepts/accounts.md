...
The `cantod debug addr <address>` can be used to convert an address between hex and bech32 formats. For example:

:::: tabs
::: tab Bech32

```bash
cantod debug addr evmos1z3t55m0l9h0eupuz3dp5t5cypyv674jj7mz2jw
...
```bash
cantod debug addr 14574A6DFF2DDF9E07828B4345D3040919AF5652
  Address: [20 87 74 109 255 45 223 158 7 130 139 67 69 211 4 9 25 175 86 82]
  Address (hex): 14574A6DFF2DDF9E07828B4345D3040919AF5652
  Bech32 Acc: evmos1z3t55m0l9h0eupuz3dp5t5cypyv674jj7mz2jw
  Bech32 Val: evmosvaloper1z3t55m0l9h0eupuz3dp5t5cypyv674jjn4d6nn
...
::: tip
The Cosmos SDK Keyring output (i.e `cantod keys`) only supports addresses and public keys in Bech32 format.
:::

We can use the `keys show` command of `cantod` with the flag `--bech <type> (acc|val|cons)` to
...
```bash
cantod keys show mykey --bech acc
- name: mykey
  type: local
...
```bash
cantod keys show mykey --bech val
- name: mykey
  type: local
...
```bash
cantod keys show mykey --bech cons
- name: mykey
  type: local
...
cantod q auth account $(cantod keys show mykey -a) -o text
|
...
To retrieve the Ethereum hex address using Web3, use the JSON-RPC [`eth_accounts`](./../../developers/api/json-rpc/endpoints.md#eth-accounts) or [`personal_listAccounts`](./../../developers/api/json-rpc/endpoints#personal-listAccounts.md) endpoints:
