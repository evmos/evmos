...
Learn how Evoblock's architecture leverages the Cosmos SDK Proof-of-Stake functionality, EVM compatibility and fast-finality from Tendermint Core's BFT consensus. {synopsis}
...
Evoblock enables the full composability and modularity of the [Cosmos SDK](https://docs.cosmos.network/).
...
engines of other blockchains. Evoblock is such an example of an ABCI application
...
Evoblock enables EVM compatibility by implementing various components that together support all the EVM state transitions while ensuring the same developer experience as Ethereum:
