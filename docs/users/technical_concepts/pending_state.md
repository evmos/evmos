...
Learn how Point network handles pending state queries. {synopsis}

## Pre-requisite Readings

- [Tendermint Mempool](https://docs.tendermint.com/master/tendermint-core/mempool/) {prereq}

## Point network vs Ethereum
...
Point network is designed quite differently on this front as there is no concept of a "pending state".
Point network uses [Tendermint Core](https://docs.tendermint.com/) BFT consensus which provides instant
...
first-in-first-out (FIFO) fashion. Transactions on Point network cannot be ordered or cherry picked out from the Tendermint node [mempool](https://docs.tendermint.com/master/tendermint-core/mempool/).

## Pending State Queries

Point network will make queries which will account for any unconfirmed transactions present in a node's
