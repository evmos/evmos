...
`hscd` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `hscd` binary, run:
...
::: tip
You can also enable auto-completion with the `hscd completion` command. For example, at the start of a bash session, run `. <(hscd completion)`, and all `hscd` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.hscd` directory.
...
To specify the `hscd` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.hscd/config`:
...
To set pruning, adjust the `pruning` parameter in the `~/.hscd/config/app.toml` file.
...
We can view the default client config setting by using `hscd config` command:
...
A list of commonly used flags of `hscd` is listed below:

| Option              | Description                   | Type         | Default Value   |
|---------------------|-------------------------------|--------------|-----------------|
| `--chain-id`        | Full Chain ID                 | String       | ---             |
| `--home`            | Directory for config and data | string       | `~/.hscd`     |
| `--keyring-backend` | Select keyring's backend      | os/file/test | os              |
| `--output`          | Output format                 | string       | "text"          |

## Command list

A list of commonly used `hscd` commands. You can obtain the full list by using the `hscd -h` command.
