...
Learn how Hazlor handles pending state queries. {synopsis}
...
Hazlor is designed quite differently on this front as there is no concept of a "pending state".
Hazlor uses [Tendermint Core](https://docs.tendermint.com/) BFT consensus which provides instant
...
first-in-first-out (FIFO) fashion. Transactions on Hazlor cannot be ordered or cherry picked out from the Tendermint node [mempool](https://docs.tendermint.com/master/tendermint-core/mempool.html#transaction-ordering).

## Pending State Queries

Hazlor will make queries which will account for any unconfirmed transactions present in a node's
...
Learn how to deploy a Solidity smart contract on Hazlor using [Truffle](./../guides/truffle.md) {hide}
