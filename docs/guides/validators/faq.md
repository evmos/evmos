...
Check the FAQ for running a validator on Hazlor {synopsis}
...
Hazlor is a public Proof-of-Stake (PoS) blockchain, meaning that validator's weight is determined by the amount of staking tokens (Planet) bonded as collateral. These staking tokens can be staked directly by the validator or delegated to them by Planet holders.
...
Delegators are Planet holders who cannot, or do not want to run validator operations themselves. Users can delegate Planets to a validator and obtain a part of its revenue in exchange (for more detail on how revenue is distributed, see [What is the incentive to stake?](#what-is-the-incentive-to-stake) and [What is a validator's commission?](#what-is-a-validators-commission) sections below).
...
- **Validator's Address**: `hazlorvaloper1-` address. This is the address used to identify your validator publicly. The private key associated with this address is used to bond, unbond, and claim rewards.
...
- **Minimum self-bond amount**: Minimum amount of Planet the validator needs to have bonded at all times. If the validator's self-bonded stake falls below this limit, its entire staking pool will be unbonded.
- **Initial self-bond amount**: Initial amount of Planet the validator wants to self-bond.

```bash
hazlord tx staking create-validator
    --pubkey hazlorvalconspub1zcjduepqs5s0vddx5m65h5ntjzwd0x8g3245rgrytpds4ds7vdtlwx06mcesmnkzly
    --amount "2ascas"
    --from tmp
...
Once a validator is created and registered, Planet holders can delegate Planets to it, effectively adding stake to its pool. The total stake of a validator is the sum of the Planet self-bonded by the validator's operator and the Planet bonded by external delegators.
...
- **Tendermint Key**: This is a unique key used to sign block hashes. It is associated with a public key `hazlorvalconspub`.

  - Generated when the node is created with `hazlord init`.
  - Get this value with `hazlord tendermint show-validator`
    e.g. `hazlorvalconspub1zcjduc3qcyj09qc03elte23zwshdx92jm6ce88fgc90rtqhjx8v0608qh5ssp0w94c`

- **Application keys**: These keys are created from the application and used to sign transactions. As a validator, you will probably use one key to sign staking-related transactions, and another key to sign oracle-related transactions. Application keys are associated with a public key `hazlorpub-` and an address `hazlor-`. Both are derived from account keys generated by `hazlord keys add`.

::: warning
A validator's operator key is directly tied to an application key, but uses reserved prefixes solely for this purpose: `hazlorvaloper` and `hazlorvaloperpub`
:::
...
- `unbonded`: Validator is not in the active set, and therefore not signing blocks. Unbonded validators cannot be slashed, but do not earn any rewards from their operation. It is still possible to delegate Planet to this validator. Un-delegating from an `unbonded` validator is immediate.

Delegators have the same state as their validator.

::: warning
Delegations are not necessarily bonded. Planet can be delegated and bonded, delegated and unbonding, delegated and unbonded, or liquid.
:::

### What is "self-bond"? How can I increase my "self-bond"?

The validator operator's "self-bond" refers to the amount of Planet stake delegated to itself. You can increase your self-bond by delegating more Planet to your validator account.
...
### Is there a minimum amount of Planet that must be staked to be an active (bonded) validator?
...
- **Amount of self-bonded Planet:** Number of Planets a validator self-bonded to its staking pool. A validator with higher amount of self-bonded Planet has more skin in the game, making it more liable for its actions.

- **Amount of delegated Planets:** Total number of Planet delegated to a validator. A high stake shows that the community trusts this validator, but it also means that this validator is a bigger target for hackers. Validators are expected to become less and less attractive as their amount of delegated Planet grows. Bigger validators also increase the centralization of the network.

- **Commission rate:** Commission applied on revenue by validators before it is distributed to their delegators

- **Track record:** Delegators will likely look at the track record of the validators they plan to delegate to. This includes seniority, past votes on proposals, historical average uptime and how often the node was compromised.

Apart from these criteria, there will be a possibility for validators to signal a website address to complete their resume. Validators will need to build reputation one way or another to attract delegators. For example, it would be a good practice for validators to have their setup audited by third parties. Note though, that the Hazlor team will not approve or conduct any audit itself.
...
- **Provide oversight and feedback on correct deployment of community pool funds:** the Hazlor protocol includes the a governance system for proposals to the facilitate adoption of its currencies. Validators are expected to hold budget executors to account to provide transparency and efficient use of funds.

Additionally, validators are expected to be active members of the community. They should always be up-to-date with the current state of the ecosystem so that they can easily adapt to any change.

### What does staking imply?

Staking Planet can be thought of as a safety deposit on validation activities. When a validator or a delegator wants to retrieve part or all of their deposit, they send an unbonding transaction. Then, Planet undergo a _three weeks unbonding period_ during which they are liable to being slashed for potential misbehaviors committed by the validator before the unbonding process started.

Validators, and by association delegators, receive block provisions, block rewards, and fee rewards. If a validator misbehaves, a certain portion of its total stake is slashed (the severity of the penalty depends on the type of misbehavior). This means that every user that bonded Planet to this validator gets penalized in proportion to its stake. Delegators are therefore incentivized to delegate to validators that they anticipate will function safely.

### Can a validator run away with its delegators' Planet?

By delegating to a validator, a user delegates staking power. The more staking power a validator has, the more weight it has in the consensus and processes. This does not mean that the validator has custody of its delegators' Planet. _By no means can a validator run away with its delegator's funds_.
...
### How often will a validator be chosen to propose the next block? Does it go up with the quantity of Planet staked?

The validator that is selected to mine the next block is called the **proposer**, the "leader" in the consensus for the round. Each proposer is selected deterministically, and the frequency of being chosen is equal to the relative total stake (where total stake = self-bonded stake + delegators stake) of the validator. For example, if the total bonded stake across all validators is 100 Planet, and a validator's total stake is 10 Planet, then this validator will be chosen 10% of the time as the proposer.
...
- **Block rewards:** Native tokens of applications run by validators (e.g. Planets on Hazlor) are inflated to produce block provisions. These provisions exist to incentivize Planet holders to bond their stake, as non-bonded Planet will be diluted over time.
- **Transaction fees:** Hazlor maintains a whitelist of token that are accepted as fee payment. The initial fee token is the `planet`.
...
Revenue received by a validator's pool is split between the validator and its delegators. The validator can apply a commission on the part of the revenue that goes to its delegators. This commission is set as a percentage. Each validator is free to set its initial commission, maximum daily commission change rate and maximum commission. Hazlor enforces the parameter that each validator sets. These parameters can only be defined when initially declaring candidacy, and may only be constrained further after being declared.

### How are block provisions distributed?

Block provisions (rewards) are distributed proportionally to all validators relative to their total stake (voting power). This means that even though each validator gains Planets with each provision, all validators will still maintain equal weight.

Let us take an example where we have 10 validators with equal staking power and a commission rate of 1%. Let us also assume that the provision for a block is 1000 Planets and that each validator has 20% of self-bonded Planet. These tokens do not go directly to the proposer. Instead, they are evenly spread among validators. So now each validator's pool has 100 Planets. These 100 Planets will be distributed according to each participant's stake:

- Commission: `100*80%*1% = 0.8 Planets`
- Validator gets: `100\*20% + Commission = 20.8 Planets`
- All delegators get: `100\*80% - Commission = 79.2 Planets`

Then, each delegator can claim its part of the 79.2 Planets in proportion to their stake in the validator's staking pool. Note that the validator's commission is not applied on block provisions. Note that block rewards (paid in Planets) are distributed according to the same mechanism.
...
Let's take a concrete example to illustrate the aforementioned concept. In this example, there are 10 validators with equal stake. Each of them applies a 1% commission and has 20% of self-bonded Planet. Now comes a successful block that collects a total of 1005 Planets in fees. Let's assume that the proposer included 100% of the signatures in its block. It thus obtains the full bonus of 5%.
...
- For the proposer validator:
  ...
  - The pool obtains $R ~ + ~ 5\%(R)$: 105 Planets
  - Commission: $105 ~ * ~ 80\% ~ * ~ 1\%$ = 0.84 Planets
  - Validator's reward: $105 ~ * ~ 20\% ~ + ~ Commission$ = 21.84 Planets
  - Delegators' rewards: $105 ~ * ~ 80\% ~ - ~ Commission$ = 83.16 Planets \(each delegator will be able to claim its portion of these rewards in proportion to their stake\)

  - The pool obtains $R$: 100 Planets
  - Commission: $100 ~ * ~ 80\% ~ * ~ 1\%$ = 0.8 Planets
  - Validator's reward: $100 ~ * ~ 20\% ~ + ~ Commission$ = 20.8 Planets
  - Delegators' rewards: $100 ~ * ~ 80\% ~ - ~ Commission$ = 79.2 Planets \(each delegator will be able to claim its portion of these rewards in proportion to their stake\)
...
### Do validators need to self-bond Planets
...
Even though there is no obligation for validators to self-bond Planet, delegators should want their validator to have self-bonded Planet in their staking pool. In other words, validators should have skin-in-the-game.

In order for delegators to have some guarantee about how much skin-in-the-game their validator has, the latter can signal a minimum amount of self-bonded Planet. If a validator's self-bond goes below the limit that it predefined, this validator and all of its delegators will unbond.

### How to prevent concentration of stake in the hands of a few top validators?

For now the community is expected to behave in a smart and self-preserving way. When a mining pool in Bitcoin gets too much mining power the community usually stops contributing to that pool. Hazlor will rely on the same effect initially. In the future, other mechanisms will be deployed to smoothen this process as much as possible:
...
In addition to running an Hazlor node, validators should develop monitoring, alerting and management solutions.

### What are bandwidth requirements?

Hazlor has the capacity for very high throughput compared to chains like Ethereum or Bitcoin.
...
The Hazlor team does not recommend one solution above the other. The community is encouraged to bolster the effort to improve HSMs and the security of key management.
