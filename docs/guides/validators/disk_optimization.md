...
speed and transaction amount. For Hazlor, we are talking about close to
...
I believe this was disabled by default on Hazlor, but listing it in any
...
will risk database corruption if the `hazlord` is killed for any reason.
...
Below is the disk usage after two weeks of Hazlor Arsia Mons testnet. The default
