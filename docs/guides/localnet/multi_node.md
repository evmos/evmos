...
This command creates a 4-node network using the `berachaindnode` Docker image.
...
Creating berachaindnode0 ... done
Creating berachaindnode2 ... done
Creating berachaindnode1 ... done
Creating berachaindnode3 ... done
```
...
calling the `berachaind testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/berachaind` directory in each container.
...
```bash
# node 0: daemon logs
docker exec berachaindnode0 tail berachaind.log
...
To interact with `berachaind` and start querying state or creating txs, you use the
`berachaind` directory of any given node as your `home`, for example:

```bash
berachaind keys list --home ./build/node0/berachaind
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/berachaind/key_seed.json` and can be restored to the CLI using the `berachaind keys add --restore` command
:::
