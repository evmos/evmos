...
See the list of supported gRPC-Gateway API endpoints for the Point Chain testnet [here](https://api.evmos.dev/).
...
Check the `Events` page to obtain the event list of each supported module on Point Chain.
:::
...
Users can use the `{{ $themeConfig.project.binary }}` binary to interact directly with an Point Chain node
though the CLI.
