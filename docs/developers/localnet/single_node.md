...
`cantod-1` and one predefined account (`mykey`) with some allocated funds at the genesis.
...
The command above creates all the configuration files needed for your node and validator to run, as well as a default genesis file, which defines the initial state of the network. All these [configuration files](./../../quickstart/binary.md#configuring-the-node) are in `~/.cantod` by default, but you can overwrite the location of this folder by passing the `--home` flag.
...
```bash
cantod add-genesis-account my_validator 10000000000aevmos
```
...
cantod add-genesis-account my_validator 1000000000stake,10000000000aevmos
```
...
3. Link the operator account with a Tendermint node pubkey that will be used for signing blocks. If no `--pubkey` flag is provided, it defaults to the local node pubkey created via the `cantod init` command above.
...
This command will add all the `gentxs` stored in `~/.cantod/config/gentx` to the genesis file.
