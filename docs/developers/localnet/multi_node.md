...
This command creates a 4-node network using the `cantodnode` Docker image.
...
Creating cantodnode0 ... done
Creating cantodnode2 ... done
Creating cantodnode1 ... done
Creating cantodnode3 ... done
```
...
calling the `cantod testnet` command. This outputs a handful of files in the
`./build` directory:
...
Each `./build/nodeN` directory is mounted to the `/cantod` directory in each container.
...
```bash
# node 0: daemon logs
docker exec cantodnode0 tail cantod.log
...
To interact with `cantod` and start querying state or creating txs, you use the
`cantod` directory of any given node as your `home`, for example:

```bash
cantod keys list --home ./build/node0/cantod
```
...
::: tip
**Note**: Each node's seed is located at `./build/nodeN/cantod/key_seed.json` and can be restored to the CLI using the `cantod keys add --restore` command
:::
