...
Learn how to deploy a simple Solidity-based smart contract to Point network using the Truffle environment {synopsis}
...
This will allow your contract to connect to your Point network local node.
...
You should see incoming deployment logs in the Point network daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the other to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the Point network node using the `test` command:
