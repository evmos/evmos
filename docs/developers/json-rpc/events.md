...
looking for. For example, a an Ethereum transaction on Point network (`MsgEthereumTx`) triggers an `event` of type `ethermint` and
...
Point network also supports the Ethereum [JSON-RPC](./server.md) filters calls to
...
Since Point network runs uses Tendermint Core as it's consensus Engine and it's built with the Cosmos
SDK framework, it inherits the event format from them. However, in order to support the native Web3
compatibility for websockets of the [Ethereum's PubSubAPI](https://geth.ethereum.org/docs/rpc/pubsub), Point network needs to cast the Tendermint
