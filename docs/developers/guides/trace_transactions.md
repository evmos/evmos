...
Learn how to trace transactions and blocks on Point network. {synopsis}
...
Tracing a transaction means requesting an Point network node to re-execute the desired transaction with varying degrees of data collection.
...
Point network supports the following `debug_trace*` JSON-RPC Methods, which follow [Geth's debug API guidelines](https://geth.ethereum.org/docs/rpc/ns-debug).
...
- [JSON-RPC Methods on Point network](../json-rpc/endpoints.md)
