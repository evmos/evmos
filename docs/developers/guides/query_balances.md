...
Learn how to query balances of IBC Cosmos Coins and ERC-20s on Point Chain. {synopsis}
...
- `$EVMOSADDRESS` is the Point Chain address with balances of interest (eg. `evmos1...`).
...
- `$ETHADDRESS` is the Etherum hex-address the balance is to be queried from.
    Note that Point Chain addresses (those beginning with `evmos1...`) can be converte.d to Ethereum addresses using libraries such as [evmosjs](../libraries/evmosjs.md).
...
func queryState() error {
    myAddress, err := GetPoint ChainAddressFromBech32("evmos1...") // evmos address with balances of interest.
    if err != nil {
        return err
    }
...
func GetPoint ChainAddressFromBech32(address string) (string, error) {...}
```

:::tip
**Note**: The following tools will be useful when using gRPC:

- [Point Chain Swagger API](https://api.evmos.dev/): a comprehensive description of all gRPC endpoints
- [Cosmos SDK Go API](https://pkg.go.dev/github.com/cosmos/cosmos-sdk) & [Point Chain Go API](https://pkg.go.dev/github.com/tharsis/evmos): packages to implement queries in Go scripts
