...
```bash
berachaind start --json-rpc.enable
```
...
```bash
berachaind start --json-rpc.api eth,txpool,personal,net,debug,web3,miner
```
...
```bash
# set gas cap to 85M
berachaind start --json-rpc.gas-cap 85000000000

# set gas cap to infinite (=0)
berachaind start --json-rpc.gas-cap 0
```
