...
func CalculateEpochMintProvision(
	...
	bTarget := params.ExponentialCalculation.BTarget // bonding target
	...
	// bondingIncentive doesn't increase beyond bonding target (0 < b < b_target)
	if bondedRatio.GTE(bTarget) {
		bondedRatio = bTarget
	}

	// bondingIncentive = 1 + max_variance - bondingRatio * (max_variance / b_target)
