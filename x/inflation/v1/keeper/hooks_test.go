	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("Case %s", tc.name), func(t *testing.T) {
			...
			if tc.expDistribution {
				...
				require.Greater(t, feePoolNew.CommunityPool.AmountOf(types.DefaultInflationDenom).BigInt().Uint64(),
					feePoolOrigin.CommunityPool.AmountOf(types.DefaultInflationDenom).BigInt().Uint64())
			} else {
				require.Equal(t, feePoolNew.CommunityPool.AmountOf(types.DefaultInflationDenom), feePoolOrigin.CommunityPool.AmountOf(types.DefaultInflationDenom))
			}
		})
	}
}
