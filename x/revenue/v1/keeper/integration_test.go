		...
		Describe("Funding community pool from precompiled contract calls", func() {
			Context("calling a precompiled registered contract with 100/0 community pool revenue", func() {
				...
				It("should transfer all tx fees to the community pool", func() {
					...
					validatorAddr, err := sdk.ValAddressFromBech32(s.validator.OperatorAddress)
					Expect(err).To(BeNil())
					...
					data, err := stakingPrecompile.ABI.Pack("delegate", common.BytesToAddress(userAddress), validatorHexAddr, big.NewInt(1e18))
					Expect(err).To(BeNil())
					res := contractInteract(
						userKey,
						&contractAddress,
