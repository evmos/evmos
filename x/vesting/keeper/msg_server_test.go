...
func (suite *KeeperTestSuite) TestOptInGovernanceClawback() {
	testCases := []struct {
		name     string
		malleate func() authtypes.AccountI
		expPass  bool
		...
		},
		{
			...
			false,
			"account not subject to vesting",
		},
		{
			"success - vesting account",
			func() authtypes.AccountI {
				from, priv := utiltx.NewAccAddressAndKey()
				baseAcc := authtypes.NewBaseAccount(from, priv.PubKey(), 1, 5)
				vestingAcc := types.NewClawbackVestingAccount(baseAcc, from, balances, suite.ctx.BlockTime(), lockupPeriods, vestingPeriods)
	...
	for _, tc := range testCases {
		...
		msg := types.NewMsgOptInGovernanceClawback(acc.GetAddress())
		res, err := suite.app.VestingKeeper.OptInGovernanceClawback(ctx, msg)

		if tc.expPass {
			suite.Require().NoError(err)
			suite.Require().NotNil(res)

			ok := s.app.VestingKeeper.HasGovClawbackEnabled(s.ctx, acc.GetAddress())
			suite.Require().True(ok)
