...
func TestParamsValidate(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:    "valid",
			params:  NewParams(DefaultEVMDenom, false, true, true, DefaultChainConfig(), extraEips, nil, nil, "", "channel-0"),
			expPass: true,
		},
		{
			name:        "empty",
...
func TestParamsEIPs(t *testing.T) {
	extraEips := []int64{2929, 1884, 1344}
	params := NewParams("ara", false, true, true, DefaultChainConfig(), extraEips, nil, nil, "", "channel-0")
	actual := params.EIPs()
