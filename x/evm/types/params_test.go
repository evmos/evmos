...
func TestParamsValidate(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:    "valid",
			params:  NewParams(DefaultEVMDenom, false, DefaultChainConfig(), extraEips, nil, nil, DefaultPermissionsPolicy),
			expPass: true,
		},
		{
			name:        "empty",
...
func TestParamsEIPs(t *testing.T) {
	extraEips := []int64{2929, 1884, 1344}
	params := NewParams("ara", false, DefaultChainConfig(), extraEips, nil, nil, DefaultPermissionsPolicy)
	actual := params.EIPs()
