...
func TestMustConvertEvmCoinTo18Decimals(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, configurator.WithEVMCoinInfo(tc.evmCoinInfo.Denom, uint32(tc.evmCoinInfo.Decimals)).Configure())
	...
	for _, cfg := range []evmtypes.EvmCoinInfo{
		...
		for _, tc := range testCases {
			t.Run(fmt.Sprintf("%d dec - %s", cfg.Decimals, tc.name), func(t *testing.T) {
				configurator := evmtypes.NewEVMConfigurator()
				configurator.ResetTestChainConfig()
				require.NoError(t, configurator.WithEVMCoinInfo(cfg.Denom, uint32(cfg.Decimals)).Configure())
