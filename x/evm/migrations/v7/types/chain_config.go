...
func (cc V6ChainConfig) EthereumConfig(chainID *big.Int) *params.ChainConfig {
...
func DefaultChainConfig() V6ChainConfig {
...
func (cc V6ChainConfig) Validate() error {
...
func validateHash(hex string) error {
	if hex != "" && strings.TrimSpace(hex) == "" {
		return errorsmod.Wrap(errors.New("invalid chain configuration"), "hash cannot be blank")
	}
...
func validateBlock(block *sdkmath.Int) error {
	...
	if block.IsNegative() {
		return errorsmod.Wrapf(
			errors.New("invalid chain configuration"), "block value cannot be negative: %s", block,
		)
	}
