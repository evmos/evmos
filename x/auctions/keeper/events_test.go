...
func TestEmitEndAuctionEvent(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(fmt.Sprintf("Case %s", tc.name), func(t *testing.T) {
			...
			require.Equal(t, common.LeftPadBytes(bidWinnerHexAddr.Bytes(), 32), ethLog.Topics[1].Bytes()) //#nosec G115 -- gosec warning about integer overflow is not relevant here
