...
func TestAuctionInfo(t *testing.T) {
	...
	ibcCoin := sdk.NewCoin("ibc/D219F3A490310B65BDC312B5A644B0D56FFF1789D894B902A49FBF9D2F560B32", math.NewInt(1))
	evmosCoin := sdk.NewCoin("aevmos", math.NewInt(1))
	anotherNativeCoin := sdk.NewCoin("aeth", math.NewInt(1))
	...
	testCases := []struct {
	...
	}{
		{
			...
			expResp: types.QueryCurrentAuctionInfoResponse{
				Tokens:        nil,
				CurrentRound:  0,
				HighestBid:    sdk.NewCoin(types.BidDenom, math.NewInt(0)),
				BidderAddress: "",
			},
			expPass: true,
		},
		{
			...
			malleate: func() {
				network.App.AuctionsKeeper.SetHighestBid(network.GetContext(), validBidderAddr.String(), sdk.NewInt64Coin(types.BidDenom, 1))
			},
			expResp: types.QueryCurrentAuctionInfoResponse{
				Tokens:        nil,
				CurrentRound:  0,
				HighestBid:    sdk.NewCoin(types.BidDenom, math.NewInt(1)),
