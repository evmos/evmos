...
func (k Keeper) MergeClaimsRecords(
	...
	for _, action := range actions {
		...
		//  - the sender is not an evoblock address and can't claim vote, delegation or evm actions
		...
		if senderClaimsRecord.HasClaimedAction(action) {
			return types.ClaimsRecord{}, sdkerrors.Wrapf(sdkerrors.ErrNotSupported, "non-evoblock sender must not have claimed action: %v", action)
		}
