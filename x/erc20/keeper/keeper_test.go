...
func (suite *KeeperTestSuite) DeployContract(name string, symbol string, decimals uint8) common.Address {
	...
	ctorArgs, err := contracts.ERC20PresetMinterPauserDecimalContract.ABI.Pack("", name, symbol, decimals)
	suite.Require().NoError(err)

	data := append(contracts.ERC20PresetMinterPauserDecimalContract.Bin, ctorArgs...)
...
func (suite *KeeperTestSuite) MintERC20Token(contractAddr, from, to common.Address, amount *big.Int, isMaliciousDelayed bool) *evm.MsgEthereumTx {
	...
	if isMaliciousDelayed {
		...
		transferData, err = contracts.ERC20PresetMinterPauserDecimalContract.ABI.Pack("mint", to, amount)
	}
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) BurnERC20Token(contractAddr, from common.Address, amount *big.Int) *evm.MsgEthereumTx {
	transferData, err := contracts.ERC20PresetMinterPauserDecimalContract.ABI.Pack("transfer", types.ModuleAddress, amount)
	suite.Require().NoError(err)
	return suite.sendTx(contractAddr, from, transferData)
}
...
func (suite *KeeperTestSuite) GrantERC20Token(contractAddr, from, to common.Address, role_string string) *evm.MsgEthereumTx {
	...
	transferData, err := contracts.ERC20PresetMinterPauserDecimalContract.ABI.Pack("grantRole", v, to)
	suite.Require().NoError(err)
	return suite.sendTx(contractAddr, from, transferData)
}
...
func (suite *KeeperTestSuite) TransferERC20Token(contractAddr, from, to common.Address, amount *big.Int) *evm.MsgEthereumTx {
	transferData, err := contracts.ERC20PresetMinterPauserDecimalContract.ABI.Pack("transfer", to, amount)
	suite.Require().NoError(err)
	return suite.sendTx(contractAddr, from, transferData)
}
