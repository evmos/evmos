...
import (
	...
	"github.com/evmos/evmos/v18/precompiles/access_control"
...
func (k Keeper) InstantiateERC20Precompile(ctx sdk.Context, contractAddr common.Address) (vm.PrecompiledContract, error) {
	...
	if pair.ContractOwner == types.OWNER_EXTERNAL {
		return accesscontrol.NewPrecompile(pair, k.bankKeeper, k.authzKeeper, *k.transferKeeper, k.accKeeper)
	}
...
func (k Keeper) RegisterERC20Extension(ctx sdk.Context, denom string) error {
	...
	return k.evmKeeper.EnableDynamicPrecompiles(ctx, pair.GetErc20Address())
}
