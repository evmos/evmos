...
func (suite *KeeperTestSuite) TestAfterProposalDeposit() {
	votingPeriod := suite.app.Erc20Keeper.GetVotingPeriod(suite.ctx, "")
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			newVotingPeriod := suite.app.Erc20Keeper.GetVotingPeriod(suite.ctx, types.ProposalTypeRegisterCoin)
