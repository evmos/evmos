...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	...
	Describe("enabled params and registered uosmo", func() {
		BeforeEach(func() {
			...
			// Register uosmo pair
			var err error
			_, err = s.EvmosChain.App.(*app.Evmos).Erc20Keeper.RegisterCoin(s.EvmosChain.GetContext(), osmoMeta)
			s.Require().NoError(err)
		})
		...
		It("should transfer and not convert aevmos", func() {
			...
			s.Require().Equal(int64(100), aevmosIBCBalanceBefore.Amount.Int64())
			...
			s.Require().Equal(int64(90), aevmosIBCSenderFinalBalance.Amount.Int64())
