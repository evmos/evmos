...
var _ = Describe("Recovery: Performing an IBC Transfer", Ordered, func() {
	...
	Describe("enabled params and registered uosmo", func() {
		...
		It("should transfer and not convert aevmos", func() {
			...
			aevmosAfterBalance := s.EvmosChain.App.(*app.Evmos).BankKeeper.GetBalance(s.EvmosChain.GetContext(), receiverAcc, "aevmos")
			...
			s.SendBackIBCCoins(s.pathOsmosisEvmos, s.IBCOsmosisChain, aevmosIbcdenom, amount, sender, receiver, 1, ibcCoinMeta)
